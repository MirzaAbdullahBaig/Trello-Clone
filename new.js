...
const createCard = (cardTitle, tickets = []) => {
    ...
    tickets.forEach(ticketText => {
        const ticket = createTicket(ticketText);
        createDiv.insertBefore(ticket, createForm);
    });
...
const addCard = (e) => {
    ...
    main.insertBefore(newCard, cardForm.parentElement);
...
const saveState = () => {
    ...
    cards.forEach(card => {
        ...
        const tickets = [];
        card.querySelectorAll('p').forEach(ticket => {
            tickets.push(ticket.textContent);
        });
        cardData.push({ title: cardTitle, tickets: tickets });
    });
...
const loadState = () => {
    ...
    if (savedCards) {
        savedCards.forEach(card => {
            const newCard = createCard(card.title, card.tickets);
            main.insertBefore(newCard, cardForm.parentElement);
        });
    }
};
